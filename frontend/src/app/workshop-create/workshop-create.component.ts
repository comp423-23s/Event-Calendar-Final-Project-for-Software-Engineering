...
export class WorkshopCreateComponent {
  ...
  onSubmit(): void{
    ...
    this.workshopCreateService
      .createWorkshop(title, description, location, date, this.profileID)
      .subscribe({
        next: (WorkshopReturned) => this.onSuccess(WorkshopReturned),
  ...
  private onSuccess(crtedWorkshop: WorkshopReturned): void{
